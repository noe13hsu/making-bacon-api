...
describe 'Categories API', type: :request do
  ...
  describe 'GET /expense_categories/' do
    it 'returns all expense categories' do
      ...
      expect(response_body).to eq(
        [
          {"description"=>"food", "id"=>19, "type"=>"expense"},
          {"description"=>"transportation", "id"=>20, "type"=>"expense"}
        ]
      )
    end
  end
  ...
  describe 'GET /income_categories/' do
    it 'returns all income categories' do
      ...
      expect(response_body).to eq(
        [
          {"description"=>"rental", "id"=>25, "type"=>"income"},
          {"description"=>"interest", "id"=>26, "type"=>"income"}
        ]
      )
    end
  end
